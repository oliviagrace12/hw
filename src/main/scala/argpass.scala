...
object argpass {
  ...
  // It has one parameter f: a function that takes an instance of RefInt (see above for the definition of the class
  // RefInt) and returns Unit (i.e., nothing interesting).
  ...
  // Your code must return a tuple of the three integers provided by f in the order that they came back from calls,
  ...
  // It will only do it after the third call to f: at that point it updates all three instances of RefInt that
  // it has received as arguments so far.
  // Your code must return a tuple of the three integers provided by f in the order that they came back from calls,
  // i.e., the integer from the first call to f is the first integer in the returned tuple.
  ...
  // It has one parameter r: (a reference to) an instance of RefInt (see above for the definition of the class RefInt)
  // and returns (a reference to) an instance of RefInt.
  // Your code must increment (add 1 to) the RefInt it receives and return double the original value
  // (stored in a separate RefInt instance) as the result.
  ...
  def refint5(xs: List[RefInt]): Unit = {
    ...
    xs.foreach((x: RefInt) => x.set(rand.nextInt()))
  }
}
