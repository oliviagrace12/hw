...
object subtyping {
  ...
  // I.e., if the increment method is called 3 times on an instance, and the decrement method is called 2 times on the same instance,
  // then it should store 5  (somewhere other than the existing field n).
  ...
  class Counter2() extends Counter {
    private var times = 0
    override def increment () = { super.increment(); times = times + 1 }
    override def decrement() = { super.increment(); times = times + 1 }
  ...
  // The first element of the result List[Int] must correspond to the number of times that increment/decrement were
  // called on the first element of type List[Counter], similarly for the second and third elements.
  def observeCounterList (f : List[Counter] => Unit) : List[Int] = {
    val counter1 = new Counter2
    val counter2 = new Counter2
    val counter3 = new Counter2

    f(counter1 :: counter2 :: counter3 :: Nil)

    counter1.getTimes :: counter2.getTimes :: counter3.getTimes :: Nil
  }
  ...
  def observeCounterArray (f : Array[Counter] => Unit) : Array[Int] = {
    ...
    f(List (counter1, counter2, counter3).toArray)

    List (counter1.getTimes(), counter2.getTimes(), counter3.getTimes()).toArray
  }
}
